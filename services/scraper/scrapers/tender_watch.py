...
def scan_single_bid_awards(conn):
    print("Scanning for single-bid awards...")
    cur = conn.cursor()
    cur.execute(
    ...
    for title, org_name, detail_url, details_json in cur:
        ...
        try:
            ...
            continue
        if num_bids == 1:
            value = details.get("Contract Value", "unspecified value")
            winner = details.get("Name of the selected bidder(s)", "unspecified vendor")
            draft = make_draft(
                title=f"Single-bid award: {title or 'Untitled tender'}",
                org_name=org_name,
                detail_url=detail_url,
                reason="only one bid was received for this award",
                extra_note=f"Contract value: {value}. Awarded to: {winner}.",
            )
            submit_draft(draft)
            count += 1
    print(f"Single-bid awards flagged: {count}")
...
def scan_short_bid_windows(conn):
    ...
    for title, org_name, detail_url, closing_date, aoc_date in cur:
        ...
        if 0 < window_days < MIN_BID_WINDOW_DAYS:
            draft = make_draft(
                title=f"Short bid window: {title or 'Untitled tender'}",
                org_name=org_name,
                detail_url=detail_url,
            ...
            submit_draft(draft)
            count += 1
    print(f"Short bid-window awards flagged: {count}")
...
def scan_repeat_winners(conn):
    ...
    count = 0
    for winner, n in winner_counts.items():
        if n >= REPEAT_WINNER_THRESHOLD:
            ...
            submit_draft(draft)
            count += 1
    print(f"Repeat-winner vendors flagged: {count}")
